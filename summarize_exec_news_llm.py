...
[bullet_points] 각 item은 **한 명의 인물(또는 한 조직개편)** 만 담음. 브리핑 스타일, 해당 인물/항목 관련 1~3개. 명사형 또는 "~함"체. **진행 단계(내정/선임/안건 등)와 실행 시기(언급 시) 포함.** 인물명은 작은따옴표(')로 감쌈. **다른 회사 출신이 타사에 선임·영입된 경우** bullet_points·요약에 반드시 "A 출신 B 사장이 C 대표로 선임" 형식(출신 회사 + 인물 + 이전 직함 + 선임된 회사 + 신규 직함)으로 쓴다. 한 기사에 여러 명이면 인물별로 나눔.
